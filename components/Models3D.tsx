 ...
 * strong emissives (picked up by the bloom pass). Somewhere between a
 * cartoon and Unreal: clean shapes, real lighting, glowing accents.
 */
...
export const MATS = {
  ...
  neonMagenta: new THREE.MeshStandardMaterial({ color: '#e879f9', emissive: '#e879f9', emissiveIntensity: 2.4, toneMapped: false }),
...
const CityModel3D: React.FC = () => (
  <group>
    ...
    <B args={[0.6, 0.035, 0.6]} mat={MATS.neonCyan} p={[0, 0.47, 0]} cast={false} />
    <B args={[0.4, 0.26, 0.4]} mat={MATS.hull} p={[0, 0.62, 0]} />
    <B args={[0.42, 0.035, 0.42]} mat={MATS.neonMagenta} p={[0, 0.76, 0]} cast={false} />
    ...
    <HoloRing radius={0.3} y={1.0} mat={MATS.neonCyan} />
...
const SolarModel3D: React.FC = () => (
  <group>
    ...
    <B args={[0.42, 0.02, 0.36]} mat={MATS.panel} p={[0.28, 0.26, 0]} r={[-0.6, 0, 0]} />
    <B args={[0.42, 0.012, 0.03]} mat={MATS.neonCyan} p={[-0.28, 0.19, 0.14]} r={[-0.6, 0, 0]} cast={false} />
    <B args={[0.42, 0.012, 0.03]} mat={MATS.neonCyan} p={[0.28, 0.19, 0.14]} r={[-0.6, 0, 0]} cast={false} />
    <Beacon p={[0, 0.32, -0.24]} mat={MATS.neonGreen} period={3} />
  </group>
);

const HabitatModel3D: React.FC = () => (
  <group>
    <DomeMesh radius={0.44} mat={MATS.hull} p={[0, 0.04, 0]} scaleY={0.8} />
    <mesh position={[0, 0.05, 0]} rotation={[-Math.PI / 2, 0, 0]} material={MATS.neonCyan}>
    ...
    <Beacon p={[0, 0.42, 0]} mat={MATS.neonMagenta} period={3} />
  </group>
);

const IceExtractorModel3D: React.FC = () => (
  <group>
    <B args={[0.42, 0.12, 0.42]} mat={MATS.dark} p={[0, 0.06, 0]} />
    <B args={[0.44, 0.025, 0.44]} mat={MATS.neonCyan} p={[0, 0.13, 0]} cast={false} />
...
const OxygenatorModel3D: React.FC = () => (
  <group>
    ...
    <mesh position={[-0.14, 0.3, 0]} rotation={[-Math.PI / 2, 0, 0]} material={MATS.neonCyan}>
      <torusGeometry args={[0.115, 0.012, 8, 24]} />
    </mesh>
    <mesh position={[0.14, 0.24, 0]} rotation={[-Math.PI / 2, 0, 0]} material={MATS.neonCyan}>
      <torusGeometry args={[0.115, 0.012, 8, 24]} />
    </mesh>
...
const GreenhouseModel3D: React.FC = () => (
  <group>
...
const MiningRigModel3D: React.FC = () => (
  <group>
    ...
    <Beacon p={[0, 0.78, 0]} mat={MATS.neonOrange} />
  </group>
);

const He3Model3D: React.FC = () => (
  <group>
    <B args={[0.44, 0.1, 0.44]} mat={MATS.dark} p={[0, 0.05, 0]} />
    ...
    <HoloRing radius={0.17} y={0.55} mat={MATS.neonMagenta} speed={-0.9} />
  </group>
);

const LaunchPadModel3D: React.FC = () => (
  <group>
    <Cyl args={[0.5, 0.54, 0.05, 24]} mat={MATS.dark} p={[0, 0.025, 0]} />
    <mesh position={[0, 0.055, 0]} rotation={[-Math.PI / 2, 0, 0]} material={MATS.neonCyan}>
    ...
    {[0, 2.1, 4.2].map(a => (
      <B key={a} args={[0.03, 0.12, 0.1]} mat={MATS.neonRed}
        p={[Math.sin(a) * 0.12, 0.1, Math.cos(a) * 0.12]} r={[0, a, 0]} />
    ))}
  </group>
);

const MODELS_3D: Record<BuildingType, React.FC> = {
  [BuildingType.CITY]: CityModel3D,
  [BuildingType.ROAD]: () => null, // rendered by the tile as flat rails
...
export const BuildingMesh: React.FC<{ type: BuildingType }> = ({ type }) => {
  const Model = MODELS_3D[type];
  return <Model />;
};

/** Hazard posts + holographic ghost shown while a rover builds. */
export const ConstructionMesh: React.FC = () => (
  <group>
    ...
    <HoloRing radius={0.34} y={0.06} mat={MATS.neonCyan} />
  </group>
);

/** Worker rover — white/gold chassis, glass cab, glowing status light. */
export const RoverMesh: React.FC<{ working: boolean }> = ({ working }) => (
  <group>
